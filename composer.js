...
function main() {
    ...
    const composer = {
        ...
        // register plugin
        register(plugin) {
            init(plugin.combinators())
            Object.assign(this, plugin.composer({ ComposerError, Composition }))
            plugins.push(plugin)
            return this
        },
    }
            ...
            encode(composition, combinators = []) { // lower non-primitive combinators by default
                ...
                const encode = composition => {
                    ...
                    if (composition.type === 'composition') {
                        ...
                        for (let plugin of plugins) code += `{ plugin: new (${plugin.constructor})(), config: ${JSON.stringify(plugin.configure())} },`
    ...
    function server() {
        ...
        for ({ plugin, config } of arguments) {
            ...
            Object.assign(compiler, plugin.compiler())
            const r = plugin.conductor(config)
            if (r._finish) finishers.push(r._finish)
            delete r._finish
